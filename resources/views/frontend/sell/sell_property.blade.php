...
?>
    <section class="form-section buying-list-page">
        <div class="container">
            <h4>{{ translateText($langtextarr,'Asuntojen osto-ilmoitukset') }}</h4>

            <div class="top-form-box">
            {{ html()->form('POST', route('frontend.sell_property'))->id("property-form")->open() }}
                ...
                <div id="filter" class="btndevice calc formfilter">
                    ...
                    <div class="form-group width-30 width-30-start">
                        ...
                        <input type="text" name="city" class="form-control" id="city"
                               placeholder="Esim. kouvola tai 02111" value="{{ @$searchData['city'] }}">
                    </div>
                    ...
                    <div class="form-group width-30">
                        ...
                        <small class="minrange">{{ translateText($langtextarr,'Min') }}: <span id="range-slider-min">{{ @$searchData['min'] }}</span> <span>€</span>
                        </small>
                        <input type="text" value="1" class="" id="myRange-min" name="min" style="display:none;">
                        <small class="maxrange">{{ translateText($langtextarr,'Max') }}: <span id="range-slider-max">{{ @$searchData['max'] }}</span> <span>€</span>
                        </small>
                    ...
                    <div class="buy-list send-btn-box">
                        <a href="javascript:void(0);" onclick="document.getElementById('property-form').submit();"
                           class="send-btn">{{ translateText($langtextarr,'Hae') }}</a>
        ...
        <div class="container">
            <div class="row-width-m">
                @if(count($propertyContacts) > 0)
                    @foreach($propertyContacts as $property)
                        <div class="width-33-n-p">
                            <a href="{{route('frontend.sell_property_details', $property->id) }}">
                            <div class="buying-list-box">
                                ...
                                <span class="price">{{ $property->property_type }}</span>
                                <div class="buying-list-description">
                                    <span><b>{{ translateText($langtextarr,'Koko') }}:</b> {{ $property->appartment_min_size }}
                                        -{{ $property->appartment_max_size }} m2</span>
                                    <br>
                                    <span><b>{{ translateText($langtextarr,'Huoneiden määrä') }}:</b> {{ $property->rooms_max }}-{{ $property->rooms_max }}
                                        pcs</span>
                                    <br>
                                    <span><b>{{ translateText($langtextarr,'Hintahaitari') }}:</b> {{ $property->appartment_min_price }}
                                        -{{ $property->appartment_max_price }} €</span>
                                    <hr>
                                    <span><b>{{ translateText($langtextarr,'Kaupunki') }}:</b> {{ $property->city }}</span>
                                    <hr>
                    ...
                    @endforeach
                @else
                    <h3>{{ translateText($langtextarr,'Tietuetta ei löytynyt') }}</h3>
                @endif
            </div>
        </div>
        <div class="clear"></div>
        <div class="container">
            {!! $propertyContacts->total() !!} {{ trans_choice('Property total|Asuntoa yhteensä', $propertyContacts->total()) }}
