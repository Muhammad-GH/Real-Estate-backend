<table class="table table-bordered table-sm taskListTable ">
    ...
    @if( $projectTask && count($projectTask) )
        <tbody>
            @if( $projectTask && count($projectTask) )
                @foreach($projectTask as $key => $projectData)
                    @include('pms.project.partials.planning_row',['project_task'=>$projectData, 'padding' =>0, 'key' => $key+1 ])
                @endforeach
            @endif
        </tbody>
        <tfoot>
